...
const Results = () => {
  ...
  const [leaderboard] = useState([
    { name: localStorage.getItem("playerName") || "You", score: finalScore, rank: 1 },
    { name: "Alex", score: Math.max(0, finalScore - 500), rank: 2 },
    { name: "Jordan", score: Math.max(0, finalScore - 1200), rank: 3 },
    { name: "Sam", score: Math.max(0, finalScore - 1800), rank: 4 },
    { name: "Taylor", score: Math.max(0, finalScore - 2500), rank: 5 },
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20 inline-block">
            ...
            <p className="text-6xl font-black text-white mb-2">{topPlayer.name}</p>
            <p className="text-4xl font-bold text-yellow-400">{topPlayer.score} points</p>
          </div>
        </div>
            ...
            {leaderboard.map((player, index) => (
              <div
                ...
                className={`bg-white/10 backdrop-blur-sm rounded-2xl p-6 border-2 transition-smooth hover:scale-105 animate-in fade-in slide-in-from-bottom-4 ${
                  player.name === (localStorage.getItem("playerName") || "You")
